...
extension BLEManager {
		...
		if let toUser = newMessage.toUser {
				...
				let num = newMessage.fromUser?.num
			{
				...
				let notification = Notification(
					...
					path: URL(
						string:
							"\(AppConstants.meowScheme):///messages?user=\(num)&id=\(newMessage.messageId)"
					)
				)
				...
				let channels = myInfo.channels?.array as? [ChannelEntity]
			{
				...
				let notification = Notification(
					...
					path: URL(
						string: "\(AppConstants.meowScheme):///messages?channel=\(channel.index)&id=\(newMessage.messageId)"
					)
				)
		...
		if fetchedWaypoint.isEmpty {
			...
			notificationManager.queue(
				notification: Notification(
					...
					path: URL(
						string: "\(AppConstants.meowScheme):///map?waypointid=\(waypoint.id)"
					)
				)
			)
		}
	...
	private func scheduleLowBatteryNotification(telemetry: TelemetryEntity) {
		...
		if let num = telemetry.nodeTelemetry?.num {
			path = URL(string: "\(AppConstants.meowScheme):///nodes?num=\(num)")
		}
		else {
			path = nil
		}
